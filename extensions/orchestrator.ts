...
export function runDelegate(request: DelegateRequest): Promise<DelegateResult> {
	...
	return new Promise((resolve) => {
		...
		const finish = (ok: boolean, diagnostic?: string) => {
			...
			resolve({
				ok,
				text: lastText || diagnostic || "Worker returned no report.",
...
export function createDelegateTool(deps: ToolDeps): ToolDefinition<typeof delegateParameters> {
	return {
		name: "delegate",
		label: "Delegate work",
		description: "Delegate a self-contained implementation or verification task to a cheaper worker.",
		promptSnippet: "delegate: assign a self-contained brief to a mid or small worker",
		promptGuidelines: ["Delegate only self-contained work, use the minimum tools, and review every worker report."],
		...
		async execute(_id, params, signal, onUpdate, ctx) {
			...
			try {
				...
				const result = await deps.runner({
					...
					signal,
					onEvent: (message) => onUpdate?.(textContent(message, { delegateId })),
				});
				deps.recordUsage({
					kind: "delegate",
					model: modelKey(spec),
